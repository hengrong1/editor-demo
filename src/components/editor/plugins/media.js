...
function tokenizeVideo(md, options) {
  function tokenizeReturn(tokens, idx) {
    ...
    if (service === 'video' && videoID) {
      return `<div class="video-file-player"><video id="video-player-${ulid()}" class="video-player" controls src="${videoID}"></video></div>`
    }

    if (service === 'audio' && videoID) {
      return `<div class="audio-file-player"><audio id="audio-player-${ulid()}"  class="audio-player" controls src="${videoID}"></audio></div>`
          }
...
export function videoPlugin(md, options) {
